...
## Ожидание внешней приёмки

Завершённая разработка после всех доступных необходимых проверок остаётся в
`In Review`, если обязательную оставшуюся приёмку может выполнить только
пользователь или внешний участник. Само ожидание не является defect/rework и
не возвращает issue в `In Progress`. Сохрани в comment/checkpoint кандидата,
проведённые проверки, оставшийся критерий, конкретную причину недоступности,
нужное внешнее действие и observable resume signal.

Такое issue остаётся в active scope, но освобождает execution lane, включая
Соло. Пересчитай весь frontier и выполняй независимые задачи и dependents с
доступным необходимым contract. Если внешняя проверка определяет неизвестный
обязательный contract либо обнаружен препятствующий defect, соответствующая
разработка остаётся заблокированной с точной причиной. Не пропускай доступные
проверки и не создавай новые Tasks ради учёта ожидания.

Не повторяй недоступную проверку без новых evidence, изменения кандидата или
внешнего сигнала. При изменении upstream повтори затронутые проверки. Перед
blocker-handoff перечитай полный scope: только отсутствие самостоятельной
работы позволяет остановиться из-за внешней приёмки. Итог объединяет все
оставшиеся проверки с issue, evidence, внешними действиями и resume signals;
отдельно показывает недоступную разработку и её причинные зависимости.
Сохраняй reason-specific объяснения blocker contract. Scope с обязательной
непройденной приёмкой не завершён; Экономичный checkpoint — отдельный выход.
