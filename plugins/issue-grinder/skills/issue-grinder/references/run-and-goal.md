...
run: точный исходный ID каждого критерия issue и общего scope → ссылка на
доказательство с проверенным кандидатом/условиями либо «нет» → оставшийся
пробел. Если ID нет, сохрани однозначную ссылку на текст критерия, не выдумывая
ID. После сжатия или возобновления восстанови карту по current contracts и
receipts, не только по последнему сообщению. Перед `Done` проверь критерии
этого issue, сохрани незакрытые обязательства остального scope; перед остановкой
или терминальным решением сверь весь применимый scope. Отсутствие доказательства
не означает выполнение. Подробный журнал остаётся по ссылке; отдельный tracker
или новые Tasks не создавай.
