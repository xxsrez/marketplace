...
Если prompt неизбежен, предложи `Да`, `Нет`, `Да всегда` через selector
capability, а при её отсутствии — теми же тремя взаимоисключающими вариантами
в структурированном вопросе. Отсутствие UI не является согласием:

- `Да` разрешает только exact action;
- `Нет` запрещает action и эквивалентный обход, после чего ищется безопасный
  альтернативный путь;
- `Да всегда` сохраняет узкую категорию из action type, data class, environment
  и ограниченного resource scope в project memory, подтверждает read-back и
  применяет её к эквивалентным операциям будущих run того же проекта.
...
Production и не расширяется на соседние категории. `Да` не переносится в новый
run; `Да всегда` не включает максимальную автономность implicit run и действует
только в сохранённой узкой категории.
