...
Узкая read-only консультация по `IG-FLOW-07` разрешена общим entrypoint:
Консультант получает конкретный вопрос и необходимый контекст, но не execution
пакет, самостоятельное исследование scope, реализацию, тесты или review.
Это service provider, а не второй исполнитель. Прямой запрет пользователя на
любых субагентов сохраняет приоритет.
