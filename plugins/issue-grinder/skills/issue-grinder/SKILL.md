...
Без явного выбора exact Luna Max → `Баланс` независимо от объёма. Для остальных
профилей небольшой/средний объём → `Соло`, обоснованно крупный →
`Классический`, независимо от числа карточек. Крупный объём требует
нескольких содержательных направлений с отдельным контекстом реализации и
проверки и существенной пользы многоагентного исполнения относительно
координации. При сомнении в объёме — `Соло`. `Экономичный` только явно.
