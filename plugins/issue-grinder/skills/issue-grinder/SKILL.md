...
одного issue. Сначала прочитай current Goal: совместимый Goal продолжай,
несовместимый не присваивай и не завершай. Если scope вырос с одного issue до
...
пользователя и точный сигнал возобновления. Только после принятого отчёта и
действующего platform blocker audit можно переводить активный Goal в `blocked`.
То есть `update_goal(status=blocked)` допустим только после обоих gates.
