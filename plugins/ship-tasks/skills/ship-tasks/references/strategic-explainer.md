# Независимая смысловая проверка комментариев ShipTask

Каждый комментарий, который создаёт ShipTask, до публикации проходит отдельного
субагента с `$ship-tasks:strategic-explainer`. Это обязательная независимая
проверка, а не рекомендация по стилю. Strategic Explainer переводит факты на
понятный человеку язык, но не выбирает факты, статус, границы работы,
полномочия, исправление или итог.
...
Форма входа свободна, но субагент должен быть независим от хода реализации: ему
не передаются прежний диалог, полный журнал инструментов или process diary.
...
Если основной агент нашёл фактическую ошибку или потерянный существенный факт,
он исправляет вход и повторяет независимую адаптацию. Самостоятельно
...
публикуется и связанный переход остаётся незавершённым.
