...
При `subagents=auto` каждый комментарий, который ShipTask собирается создать, до
публикации проходит отдельного независимого
...
При явном общем `subagents=off` основной агент применяет тот же quality contract
напрямую и не заявляет о независимой проверке.
...
При `subagents=auto` до публикации основной агент передаёт отдельному Strategic
Explainer точные факты, evidence, границу знания, пользовательское влияние и
следующий шаг. Explainer возвращает готовый текст на языке пользователя.
...
Если при `subagents=auto` отдельный Explainer недоступен или не дал пригодный
текст, комментарий не публикуется. Связанный существенный переход остаётся
незавершённым. Это capability failure, а не opt-out. При общем `subagents=off`
основной агент формирует grounded problem-first comment напрямую. Ответ в Codex
